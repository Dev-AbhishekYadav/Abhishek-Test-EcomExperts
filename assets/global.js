...
/**
 *  CUSTOM WEB COMPONENT CREATED FOR SWATCHES PRODUCTS
 */

class VariantSelectsSwatches extends HTMLElement {
  constructor() {
    super();
    this.addEventListener('change', this.onVariantChange);
    this.preselectFirstVariant();
  ...
  addEventToSwatches(){
    document.getElementById('swatches-picker').addEventListener('click', e => {
        if(e.target.classList.contains('variant-swatch')){
            let swatch =  e.target;
            let color = swatch.dataset.color;
            if(swatch.classList.contains('active')) return;
            this.manageThumbnails(color);
            this.querySelector('[name="options[Color]"]').value = color;
            document.querySelector('#swatches-picker span.active').classList.remove('active');
  ...
  manageThumbnails(color){
    color = color.toLowerCase();
    const thumbnailCarousel = document.querySelector('[id*="Slider-Thumbnails"]');
    thumbnailCarousel.querySelectorAll('li').forEach(slide => slide.classList.contains(color) ? slide.classList.remove('hide') : slide.classList.add('hide'))
  }
...
customElements.define('variant-selects-swatches', VariantSelectsSwatches);
